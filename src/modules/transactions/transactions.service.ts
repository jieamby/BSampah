...
export class TransactionsService {
  async scanBarcode(barcode: string) {
    const customer = await db
      .select()
      .from(customers)
      .where(eq(customers.barcode, barcode));
  ...
  async create(dto: CreateTransactionDto) {
    const customer = await this.scanBarcode(dto.barcode);
    ...
    await db
      .update(customers)
      .set({
        balance: sql`${customers.balance} + ${totalAmount}`,
      })
  ...
  // ... findAll, findOne, findByCustomer tetap sama ...
